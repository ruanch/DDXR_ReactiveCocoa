...
#import "JaredCityGroupCell.h"
#import "JaredCityHeaderView.h"
#import <CoreLocation/CoreLocation.h>
#import <MapKit/MapKit.h>

@interface JaredChooseCityController ()<JaredCityGroupCellDelegate,UISearchBarDelegate,CLLocationManagerDelegate>
/**
 *  记录所有城市信息，用于搜索
 */
...
@implementation JaredChooseCityController
...
-(void)viewDidLoad{
    ...
    [self.tableView registerClass:[JaredCityGroupCell class] forCellReuseIdentifier:cityGroupCell];
    [self.tableView registerClass:[JaredCityHeaderView class] forHeaderFooterViewReuseIdentifier:cityHeaderView];
}
...
- (NSMutableArray *) localCityData
{
    if (_localCityData == nil) {
        ...
        if (self.locationCityID != nil) {
            JaredCity *city = nil;
            for (JaredCity *item in self.recordCityData) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    JaredCityGroup *group = [self.cityDatas objectAtIndex:section - 3];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isSearch) {
        ...
        JaredCity *city =  [self.searchCities objectAtIndex:indexPath.row];
        [cell.textLabel setText:city.cityName];
        return cell;
    }
    
    if (indexPath.section < 3) {
        JaredCityGroupCell *cell = [tableView dequeueReusableCellWithIdentifier:cityGroupCell];
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
    JaredCityGroup *group = [self.cityDatas objectAtIndex:indexPath.section - 3];
    JaredCity *city =  [group.arrayCitys objectAtIndex:indexPath.row];
    [cell.textLabel setText:city.cityName];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    JaredCityHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:cityHeaderView];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        return [JaredCityGroupCell getCellHeightOfCityArray:self.localCityData];
    }
    else if (indexPath.section == 1) {
        return [JaredCityGroupCell getCellHeightOfCityArray:self.commonCityData];
    }
    else if (indexPath.section == 2){
        return [JaredCityGroupCell getCellHeightOfCityArray:self.hotCityData];
    }
    return 44.0f;
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if (searchText.length == 0) {
        ...
        self.isSearch = YES;
        for (JaredCity *city in self.recordCityData){
...
- (void) didSelctedCity:(JaredCity *)city
{
