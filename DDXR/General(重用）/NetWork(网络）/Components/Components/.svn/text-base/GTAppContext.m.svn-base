...
{
    if (_m == nil) {
        _m = [[self.device.model stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] GT_defaultValue:@""];
    }
    return _m;
}
...
{
    if (_o == nil) {
        _o = [[self.device.systemName stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] GT_defaultValue:@""];
    }
    return _o;
}
 - (NSString *)v
...
{
    if (_cv == nil) {
        _cv = [[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"] GT_defaultValue:@""];
    }
    return _cv;
}
...
{
    if (_macid == nil) {
        _macid = [[self.device GT_macaddressMD5] GT_defaultValue:@""];
    }
    return _macid;
}
...
{
    if (_uuid == nil) {
        _uuid = [[self.device GT_uuid] GT_defaultValue:@""];
    }
    return _uuid;
}
...
{
    if (_ostype2 == nil) {
        _ostype2 = [self.device.GT_ostype GT_defaultValue:@""];
    }
    return _ostype2;
}
...
{
    if (_uuid2 == nil) {
        _uuid2 = [self.device.GT_uuid GT_defaultValue:@""];
    }
    return _uuid2;
}
...
{
    if (_udid2 == nil) {
        _udid2 = [self.device.GT_uuid GT_defaultValue:@""];
    }
    return _udid2;
}
...
- (NSString *)cid
{
    return [[GTLocationManager sharedInstance] currentCityId];
}
...
- (NSString *)geo
{
    CLLocationCoordinate2D coordinate = [[GTLocationManager sharedInstance].locatedCityLocation coordinate];
    return [NSString stringWithFormat:@"%f, %f", coordinate.latitude, coordinate.longitude];
}

- (NSString *)gcid
{
    return [GTLocationManager sharedInstance].locatedCityId;
}
...
{
    if (_pmodel == nil) {
        _pmodel = [[UIDevice currentDevice] GT_machineType];
    }
    return _pmodel;
}
...
{
    static GTAppContext *sharedInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[GTAppContext alloc] init];
        [[AFNetworkReachabilityManager sharedManager] startMonitoring];
        [GTLocationManager sharedInstance];
    });
    return sharedInstance;
}

- (void)configWithChannelID:(NSString *)channelID appName:(NSString *)appName appType:(GTAppType)appType ccid:(NSString *)ccid
{
    self.channelID = channelID;
    self.appName = appName;
    self.appType = appType;
    self.ccid = ccid;
    [[GTLogger sharedInstance].configParams configWithAppType:appType];
}
