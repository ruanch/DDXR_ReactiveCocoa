...
@class GTAPIBaseManager;

// 在调用成功之后的params字典里面，用这个key可以取出requestID
static NSString * const kGTAPIBaseManagerRequestID = @"kGTAPIBaseManagerRequestID";
...
/*************************************************************************************************/
/*                               GTAPIManagerApiCallBackDelegate                                 */
/*************************************************************************************************/

//api回调
@protocol GTAPIManagerApiCallBackDelegate <NSObject>
 @required
...
/*************************************************************************************************/
/*                               GTAPIManagerCallbackDataReformer                                */
/*************************************************************************************************/
//负责重新组装API数据的对象
/*
 重组数据的场景：
 ...
 */
@protocol GTAPIManagerCallbackDataReformer <NSObject>
 @required
/*
 ...
 代码样例：
    - (id)manager:(GTAPIBaseManager *)manager reformData:(NSDictionary *)data
    {
...
/*
 使用场景：
        ...
        3.有一种情况是manager调用api时使用的参数不一定是明文传递的，有可能是从某个变量或者跨越了好多层的对象中来获得参数，那么在调用api的最后一关会有一个参数验证，当参数不对时不访问api，同时自身的errorType将会变为GTAPIManagerErrorTypeParamsError。这个机制可以优化我们的app。
 ...
 */
@protocol GTAPIManagerValidator <NSObject>
 @required
/*
...
/*************************************************************************************************/
/*                                GTAPIManagerParamSourceDelegate                                */
/*************************************************************************************************/
//让manager能够获取调用API所需要的数据
@protocol GTAPIManagerParamSourceDelegate <NSObject>
 @required
 ...
 */
typedef NS_ENUM (NSUInteger, GTAPIManagerErrorType){
    GTAPIManagerErrorTypeDefault,       //没有产生过API请求，这个是manager的默认状态。
    GTAPIManagerErrorTypeSuccess,       //API请求成功且返回数据正确，此时manager的数据是可以直接拿来使用的。
    GTAPIManagerErrorTypeNoContent,     //API请求成功但返回数据不正确。如果回调数据验证函数返回值为NO，manager的状态就会是这个。
    GTAPIManagerErrorTypeParamsError,   //参数错误，此时manager不会调用API，因为参数验证是在调用API之前做的。
    GTAPIManagerErrorTypeTimeout,       //请求超时。GTAPIProxy设置的是20秒超时，具体超时时间的设置请自己去看GTAPIProxy的相关代码。
    GTAPIManagerErrorTypeNoNetWork      //网络不通。在调用API之前会判断一下当前网络是否通畅，这个也是在调用API之前验证的，和上面超时的状态是有区别的。
};

typedef NS_ENUM (NSUInteger, GTAPIManagerRequestType){
    GTAPIManagerRequestTypeGet,
    GTAPIManagerRequestTypePost,
    GTAPIManagerRequestTypeRestGet,
    GTAPIManagerRequestTypeRestPost
};
...
/*************************************************************************************************/
/*
 GTAPIBaseManager的派生类必须符合这些protocal
 */
@protocol GTAPIManager <NSObject>

@required
- (NSString *)methodName;
- (NSString *)serviceType;
- (GTAPIManagerRequestType)requestType;
...
/*************************************************************************************************/
/*                                    GTAPIManagerInterceptor                                    */
/*************************************************************************************************/
/*
 GTAPIBaseManager的派生类必须符合这些protocal
 */
@protocol GTAPIManagerInterceptor <NSObject>

@optional
- (void)manager:(GTAPIBaseManager *)manager beforePerformSuccessWithResponse:(GTURLResponse *)response;
- (void)manager:(GTAPIBaseManager *)manager afterPerformSuccessWithResponse:(GTURLResponse *)response;

- (void)manager:(GTAPIBaseManager *)manager beforePerformFailWithResponse:(GTURLResponse *)response;
- (void)manager:(GTAPIBaseManager *)manager afterPerformFailWithResponse:(GTURLResponse *)response;

- (BOOL)manager:(GTAPIBaseManager *)manager shouldCallAPIWithParams:(NSDictionary *)params;
- (void)manager:(GTAPIBaseManager *)manager afterCallingAPIWithParams:(NSDictionary *)params;
...
/*                                       GTAPIBaseManager                                        */
/*************************************************************************************************/
@interface GTAPIBaseManager : NSObject

@property (nonatomic, weak) id<GTAPIManagerApiCallBackDelegate> delegate;
@property (nonatomic, weak) id<GTAPIManagerParamSourceDelegate> paramSource;
@property (nonatomic, weak) id<GTAPIManagerValidator> validator;
@property (nonatomic, weak) NSObject<GTAPIManager> *child; //里面会调用到NSObject的方法，所以这里不用id
@property (nonatomic, weak) id<GTAPIManagerInterceptor> interceptor;
...
@property (nonatomic, readonly) GTAPIManagerErrorType errorType;
...
/*
 ...
 GTAPIBaseManager会先调用这个函数，然后才会调用到 id<GTAPIManagerValidator> 中的 manager:isCorrectWithParamsData:
