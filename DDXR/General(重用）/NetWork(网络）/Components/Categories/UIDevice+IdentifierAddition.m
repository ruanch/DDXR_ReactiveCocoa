...
- (NSString *)GT_uuid
{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self GT_createUUID] forKey:key];
...
- (NSString *) GT_udid
{
    NSString *udid = [[GTUDIDGenerator sharedInstance] UDID];
    if (udid.length==0) {
        udid = [self GT_uuid];
        [[GTUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *) GT_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] GT_macaddress];
...
- (NSString *) macaddress
{
    return self.GT_macaddress;
}
- (NSString *) macaddressMD5
{
    return self.GT_macaddressMD5;
}
- (NSString *) machineType
{
    return self.GT_machineType;
}
- (NSString *) ostype//显示“ios6，ios5”，只显示大版本号
{
    return self.GT_ostype;
}
- (NSString *) createUUID
{
    return self.GT_createUUID;
}
